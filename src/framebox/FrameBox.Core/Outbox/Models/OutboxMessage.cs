...
public class OutboxMessage : IMessage
{
    ...
    public async Task<IEvent> ToDomainEvent(IEventRegistry eventRegistry, IDomainEventSerializer domainEventSerializer, CancellationToken cancellationToken)
    {
        var eventType = eventRegistry.GetEventType(EventType) ?? throw new InvalidOperationException("Type not found!");
